...
public:
  ...
  void draw(World &monde, const Cairo::RefPtr<Cairo::Context> &cr, int height,
            int width);
  bool is_invalid(std::vector<std::unique_ptr<Corail>> const &tab_corail);

  std::unique_ptr<Corail> reproduction(int new_id);

  bool read_segment_invalid(std::string line);
  void add_segment();
  void erase_last_segment();
  void replace_effector(const Segment &seg);
  bool collision(double epsilon,
                 std::vector<std::unique_ptr<Corail>> const &tab_corail,
  ...
  std::vector<Algue *>
  get_algue_with_distance_max_lrepro(std::vector<Algue> &tab_algue);
  int get_id() const;
  bool get_disponibility() const;
  S2d get_extremity() const;
  Segment get_last_segment() const;
  std::vector<Segment> get_tab_segments() const;
...
private:
  int id = 0;
  Statut_cor statut_cor = DEAD;
  Dir_rot_cor dir_rot = INVTRIGO;
  Statut_dev_cor statut_dev = EXTEND;
  std::vector<Segment> segments;
  bool available = true;

  void decode(std::string line);
  bool segment_field_invalid(double epsilon);
  bool coral_id_is_invalid(
      std::vector<std::unique_ptr<Corail>> const &tab_corail);
  ...
  bool segments_intersection_inter_corail(
      std::vector<std::unique_ptr<Corail>> const &tab_corail,
      double epsilon) const;
  bool segments_intersection_intra_corail(double epsilon) const;
  bool segment_intersection_inter_corail(
      int id, int segment_index, const Segment &segment,
      std::vector<std::unique_ptr<Corail>> const &tab_corail,
      double epsilon) const;
};
...
public:
  ...
  void draw(World &monde, const Cairo::RefPtr<Cairo::Context> &cr, int height,
            int width);
  bool is_invalid(std::vector<std::unique_ptr<Corail>> &tab_corail);
  ...
  int get_id_target() const;

private:
  Statut_sca status_scavenger = LIBRE;
  int radius = 0;
  int coral_id_target = 0;
  Corail *coral_target = nullptr;

  void decode(std::string line);
  bool radius_invalid() const;
  bool id_target_invalid(std::vector<std::unique_ptr<Corail>> &tab_corail);
};
