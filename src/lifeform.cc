...
S2d Lifeform::get_position() const { return position; }
...
// Algue
Algue::Algue(int position_x, int position_y)
{
...
Algue::Algue(std::string line) { decode(line); }
...
// Corail
Corail::Corail(std::string line) { decode(line); }

Corail::Corail(Segment seg, int new_id)
{
  id = new_id;
  age = 1;
  statut_cor = ALIVE;
...
// fonctions get
std::vector<Algue *>
Corail::get_algue_with_distance_max_lrepro(std::vector<Algue> &tab_algue)
{
  std::vector<Algue *> algue_distance_max_lrepro;
  for (size_t i = 0; i < tab_algue.size(); ++i)
  {
    ...
    {
      algue_distance_max_lrepro.push_back(&tab_algue[i]);
    }
  }
...
bool Corail::get_disponibility() const { return available; }

S2d Corail::get_extremity() const { return segments.back().extremity; }

Segment Corail::get_last_segment() const { return segments.back(); }

std::vector<Segment> Corail::get_tab_segments() const { return segments; }
...
// Scavenger
Scavenger::Scavenger(std::string line) { decode(line); }
...
int Scavenger::get_radius() const { return radius; }

Statut_sca Scavenger::get_statut_sca() const { return status_scavenger; }

int Scavenger::get_id_target() const { return coral_id_target; }
